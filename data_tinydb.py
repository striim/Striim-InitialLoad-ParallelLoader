...
def write_to_tinydb(query_results):
    with TinyDB(config.TINYDB_PATH) as db:
        Record = Query()
...
def read_from_tinydb(where_clause_str: str) -> List[QueryResult]:
    with TinyDB(config.TINYDB_PATH) as db:
        ...
        if conditions:
            ...
            print(f"Warning: Unhandled or complex where_clause in read_from_tinydb: '{where_clause_str}'. For safety, returning no results.")
            results_from_db = []
